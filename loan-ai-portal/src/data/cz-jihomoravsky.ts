...
export interface JihomoravskyRegionContent {
  name: string
  slug: string
  heroTitle: string
  heroSubtitle: string
...
export interface JihomoravskyCityContent {
  slug: string
  name: string
  regionName: string
  countryName: string
...
const baseRegulations: string[] = [
  ...
  'Jihomoravský kraj vyžaduje transparentní RPSN u nabídek.',
...
const citySeeds = [
  {
    name: 'Brno',
    focus: 'IT a univerzity',
  },
  {
    name: 'Znojmo',
    focus: 'vinařství a turismus',
  },
  {
    name: 'Hodonín',
    focus: 'zemědělství',
...
const buildTitle = (name: string) => {
  return `Půjčky v ${name}, Jihomoravský kraj – Srovnání {{year}}`
}
...
const buildIntro = (name: string, focus: string) => {
  return `${name} v Jihomoravském kraji se specializuje na ${focus}. Poskytovatelé ověřují registry a sledují cash flow. Loan AI Portál nabídne licencované poskytovatele s transparentním RPSN.`
}
...
const jihomoravskyCityContentBase = citySeeds.map((seed, index) => {
  ...
  return {
    ...
    offer: {
      affiliate: 10100,
      campaign: slug,
      partnerName: 'Volsor Jihomoravský kraj',
      product: 'personal',
      theme: 'old_styled',
    },
  ...
  } satisfies JihomoravskyCityContent
})

export const jihomoravskyCityContent: JihomoravskyCityContent[] = jihomoravskyCityContentBase

export const jihomoravskyCityMap: Record<string, JihomoravskyCityContent> = jihomoravskyCityContent.reduce(
  (acc, city) => {
    acc[city.slug] = city
    return acc
  },
  {} as Record<string, JihomoravskyCityContent>,
)

export const jihomoravskyCitySlugs = jihomoravskyCityContent.map((city) => city.slug)

export const getJihomoravskyCity = (slug: string) => jihomoravskyCityMap[slug]

export const resolveJihomoravskyMetadata = (slug: string, year: number = CURRENT_YEAR) => {
  const city = getJihomoravskyCity(slug)
  if (!city) return null
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `/cz/regions/jihomoravsky/${slug}`,
    },
  }
}

export const jihomoravskyRegionContent: JihomoravskyRegionContent = {
  name: 'Jihomoravský kraj',
  slug: 'jihomoravsky',
  heroTitle: 'Půjčky v Jihomoravském kraji – licencované srovnání',
  heroSubtitle:
    'AI model sleduje sazby, dotace i dostupnost poboček.',
  overview:
    'Jihomoravský kraj kombinuje různé ekonomické sektory. Poskytovatelé vyžadují detailní rozpočty.',
