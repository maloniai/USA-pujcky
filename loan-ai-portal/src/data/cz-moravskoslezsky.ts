...
export interface MoravskoslezskyRegionContent {
  name: string
  slug: string
  heroTitle: string
  heroSubtitle: string
...
export interface MoravskoslezskyCityContent {
  slug: string
  name: string
  regionName: string
  countryName: string
...
const baseRegulations: string[] = [
  ...
  'Moravskoslezský kraj vyžaduje transparentní RPSN u nabídek.',
...
const citySeeds = [
  {
    name: 'Ostrava',
    focus: 'hutnictví a těžký průmysl',
  },
  {
    name: 'Opava',
    focus: 'služby',
...
const buildTitle = (name: string) => {
  return `Půjčky v ${name}, Moravskoslezský kraj – Srovnání {{year}}`
}
...
const buildIntro = (name: string, focus: string) => {
  return `${name} v Moravskoslezském kraji se specializuje na ${focus}. Poskytovatelé ověřují registry a sledují cash flow. Loan AI Portál nabídne licencované poskytovatele s transparentním RPSN.`
}
...
const moravskoslezskyCityContentBase = citySeeds.map((seed, index) => {
  ...
  return {
    ...
    offer: {
      affiliate: 10400,
      campaign: slug,
      partnerName: 'Volsor Moravskoslezský kraj',
      product: 'personal',
      theme: 'old_styled',
    },
  ...
  } satisfies MoravskoslezskyCityContent
})

export const moravskoslezskyCityContent: MoravskoslezskyCityContent[] = moravskoslezskyCityContentBase

export const moravskoslezskyCityMap: Record<string, MoravskoslezskyCityContent> = moravskoslezskyCityContent.reduce(
  (acc, city) => {
    acc[city.slug] = city
    return acc
  },
  {} as Record<string, MoravskoslezskyCityContent>,
)

export const moravskoslezskyCitySlugs = moravskoslezskyCityContent.map((city) => city.slug)

export const getMoravskoslezskyCity = (slug: string) => moravskoslezskyCityMap[slug]

export const resolveMoravskoslezskyMetadata = (slug: string, year: number = CURRENT_YEAR) => {
  const city = getMoravskoslezskyCity(slug)
  if (!city) return null
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `/cz/regions/moravskoslezsky/${slug}`,
    },
  }
}

export const moravskoslezskyRegionContent: MoravskoslezskyRegionContent = {
  name: 'Moravskoslezský kraj',
  slug: 'moravskoslezsky',
  heroTitle: 'Půjčky v Moravskoslezském kraji – licencované srovnání',
  heroSubtitle:
    'AI model sleduje sazby, dotace i dostupnost poboček.',
  overview:
    'Moravskoslezský kraj kombinuje různé ekonomické sektory. Poskytovatelé vyžadují detailní rozpočty.',
