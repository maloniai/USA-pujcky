...
export interface PardubickyRegionContent {
  name: string
  slug: string
  heroTitle: string
  heroSubtitle: string
...
export interface PardubickyCityContent {
  slug: string
  name: string
  regionName: string
  countryName: string
...
const baseRegulations: string[] = [
  ...
  'Pardubický kraj vyžaduje transparentní RPSN u nabídek.',
...
const citySeeds = [
  ...
  {
    name: 'Svitavy',
    focus: 'textil a zemědělství',
  },
]

const buildTitle = (name: string) => {
  return `Půjčky v ${name}, Pardubický kraj – Srovnání {{year}}`
}
...
const buildIntro = (name: string, focus: string) => {
  return `${name} v Pardubickém kraji se specializuje na ${focus}. Poskytovatelé ověřují registry a sledují cash flow. Loan AI Portál nabídne licencované poskytovatele s transparentním RPSN.`
}
...
const pardubickyCityContentBase = citySeeds.map((seed, index) => {
  ...
  return {
    ...
    offer: {
      affiliate: 9900,
      campaign: slug,
      partnerName: 'Volsor Pardubický kraj',
      product: 'personal',
      theme: 'old_styled',
    },
  ...
  } satisfies PardubickyCityContent
})

export const pardubickyCityContent: PardubickyCityContent[] = pardubickyCityContentBase

export const pardubickyCityMap: Record<string, PardubickyCityContent> = pardubickyCityContent.reduce(
  (acc, city) => {
    acc[city.slug] = city
    return acc
  },
  {} as Record<string, PardubickyCityContent>,
)

export const pardubickyCitySlugs = pardubickyCityContent.map((city) => city.slug)

export const getPardubickyCity = (slug: string) => pardubickyCityMap[slug]

export const resolvePardubickyMetadata = (slug: string, year: number = CURRENT_YEAR) => {
  const city = getPardubickyCity(slug)
  if (!city) return null
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `/cz/regions/pardubicky/${slug}`,
    },
  }
}

export const pardubickyRegionContent: PardubickyRegionContent = {
  name: 'Pardubický kraj',
  slug: 'pardubicky',
  heroTitle: 'Půjčky v Pardubickém kraji – licencované srovnání',
  heroSubtitle:
    'AI model sleduje sazby, dotace i dostupnost poboček.',
  overview:
    'Pardubický kraj kombinuje různé ekonomické sektory. Poskytovatelé vyžadují detailní rozpočty.',
