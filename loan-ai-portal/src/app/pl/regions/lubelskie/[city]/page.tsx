...
import { lubelskieCities } from '@/data/pl-lubelskie'
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const cityData = lubelskieCities[params.city]
  ...
  return {
    ...
    alternates: {
      canonical: `/pl/regions/lubelskie/${params.city}`,
    },
    openGraph: {
      title: cityData.metaTitleTemplate,
      description: cityData.metaDescriptionTemplate,
      url: `/pl/regions/lubelskie/${params.city}`,
      locale: 'pl_PL',
      type: 'website',
    },
    keywords: [
...
export default function CityPage({ params }: PageProps) {
  ...
  const breadcrumbItems = [
    ...
    { name: `Pożyczki ${cityData.regionName}`, url: '/pl/regions/lubelskie', position: 3 },
    { name: `Pożyczki ${cityData.name}`, url: `/pl/regions/lubelskie/${params.city}`, position: 4 },
  ]
        ...
        <header className="border-b border-blue-100 bg-gradient-to-br from-blue-50 via-white to-blue-100">
          <div className="mx-auto max-w-6xl px-4 py-12">
            <nav className="mb-4 flex items-center gap-2 text-sm text-blue-600">
              ...
              <Link href="/pl/regions/lubelskie" className="hover:underline">{cityData.regionName}</Link>
              <span aria-hidden>›</span>
...
export function generateStaticParams() {
  return Object.keys(lubelskieCities).map((city) => ({
    city,
  }))
}
