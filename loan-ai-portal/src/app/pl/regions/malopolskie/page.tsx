...
import { malopolskieRegionContent } from '@/data/pl-malopolskie'

const currentYear = new Date().getFullYear()
const canonicalPath = '/pl/regions/malopolskie'
...
const {
  ...
  topCities,
  blogTeasers,
} = malopolskieRegionContent

const breadcrumbItems = [
  { name: 'Strona główna', url: '/', position: 1 },
  { name: 'Pożyczki Polska', url: '/pl', position: 2 },
  { name: 'Pożyczki Małopolskie', url: canonicalPath, position: 3 },
]
        ...
        <header className="border-b border-blue-100 bg-gradient-to-br from-blue-50 via-white to-blue-100">
          <div className="mx-auto max-w-6xl px-4 py-16">
            <p className="text-sm font-semibold uppercase tracking-wide text-blue-700">
              Małopolskie • Przewodnik regionalny {currentYear}
            </p>
          ...
          <div className="mx-auto max-w-6xl px-4">
            ...
            <div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {topCities.map((city) => (
                <Link
                  key={city.slug}
                  href={`/pl/regions/malopolskie/${city.slug}`}
