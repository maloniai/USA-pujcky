...
import type { ZlinskyCityContent } from '@/data/cz-zlinsky'
import { getzlinskyCity, zlinskyCityMap, zlinskyCitySlugs, resolvezlinskyMetadata } from '@/data/cz-zlinsky'

interface ZlinskyCityPageProps {
  params: {
    city: string
  }
}

const canonicalRegionPath = '/cz/regions/zlinsky'

export function generateStaticParams() {
  return zlinskyCitySlugs.map((slug) => ({ city: slug }))
}

export function generateMetadata({ params }: ZlinskyCityPageProps): Metadata {
  const meta = resolvezlinskyMetadata(params.city)

  if (!meta) {
    return {
      title: 'Půjčky Zlínský kraj – města',
      description: 'Lokální srovnání půjček v Zlínský kraj_LOCATIVE s licencovanými poskytovateli.',
    }
  }
...
export default function ZlinskyCityPage({ params }: ZlinskyCityPageProps) {
  const city = getzlinskyCity(params.city)
  ...
  const nearbyCities = city.nearby
    .map((slug) => zlinskyCityMap[slug])
    .filter((entry): entry is ZlinskyCityContent => Boolean(entry))

  const currentYear = new Date().getFullYear()
  const breadcrumbItems = [
    { name: 'Domů', url: '/', position: 1 },
    { name: 'Půjčky Česko', url: '/cz', position: 2 },
    { name: 'Půjčky Zlínský kraj', url: canonicalRegionPath, position: 3 },
