...
import { olomouckyCities, olomouckyRegionContent } from '@/data/cz-olomoucky';
import { notFound } from 'next/navigation';

const currentYear = new Date().getFullYear();

export async function generateStaticParams() {
  return olomouckyCities.map((city) => ({
    city: city.slug,
  }));
}

export async function generateMetadata({ params }: { params: { city: string } }): Promise<Metadata> {
  const cityData = olomouckyCities.find(c => c.slug === params.city);
  ...
  return {
    title: `Půjčky v ${cityData.name}, Olomoucký kraj - Srovnání ${currentYear}`,
    ...
    keywords: `půjčky ${cityData.name}, úvěr ${cityData.name}, nebankovní půjčka, RPSN, Olomoucký kraj`,
    openGraph: {
      title: `Půjčky v ${cityData.name}, Olomoucký kraj - Srovnání ${currentYear}`,
...
export default function OlomouckyCityPage({ params }: { params: { city: string } }) {
  const cityData = olomouckyCities.find(c => c.slug === params.city);
  ...
  const financialServiceSchema = {
    ...
    "description": `Srovnání půjček a úvěrů v ${cityData.name}, Olomoucký kraj`,
    "areaServed": {
      "@type": "City",
      "name": cityData.name,
      "containedInPlace": {
      ...
      "itemListElement": [
        {
          ...
          "itemOffered": {
            ...
            "amount": {
              "@type": "MonetaryAmount",
              "currency": "CZK",
              "minValue": cityData.loanAmountRange.min,
              "maxValue": cityData.loanAmountRange.max
            }
          }
        }
      ]
    ...
    "potentialAction": {
      ...
      "target": {
        "@type": "EntryPoint",
        "urlTemplate": `https://loan-platform.com/cz/regions/olomoucky/${cityData.slug}`,
        "actionPlatform": [
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      },
      {
        ...
        "position": 4,
        "name": cityData.name,
        "item": `https://loan-platform.com/cz/regions/olomoucky/${cityData.slug}`
      }
    ]
  };
      ...
      <header className="bg-gradient-to-r from-blue-600 to-blue-700 text-white py-12">
        <div className="container mx-auto px-4">
          <nav className="text-sm mb-4 text-blue-100">
            ...
            <span className="mx-2">›</span>
            <Link href="/cz/regions/olomoucky" className="hover:text-white">Olomoucký kraj</Link>
            <span className="mx-2">›</span>
        ...
        <Card className="shadow-xl bg-gradient-to-r from-blue-50 to-indigo-50">
          <CardContent className="p-8">
            ...
            <div className="flex justify-center mb-6">
              <a
                href={`https://track.fibonad.com/click?campaign_id=${9340 + olomouckyCities.findIndex(c => c.slug === cityData.slug)}&pub_id=PUBLISHER_ID`}
                target="_blank"
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid md:grid-cols-3 lg:grid-cols-6 gap-4">
            {cityData.nearbyCities.map((nearbyCity, index) => {
              const nearbyData = olomouckyCities.find(c => c.name === nearbyCity);
              return (
                <Link key={index} href={`/cz/regions/olomoucky/${nearbyData?.slug || ''}`}>
