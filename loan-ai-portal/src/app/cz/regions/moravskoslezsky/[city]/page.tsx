...
import type { MoravskoslezskyCityContent } from '@/data/cz-moravskoslezsky'
import { getmoravskoslezskyCity, moravskoslezskyCityMap, moravskoslezskyCitySlugs, resolvemoravskoslezskyMetadata } from '@/data/cz-moravskoslezsky'

interface MoravskoslezskyCityPageProps {
  params: {
    city: string
  }
}

const canonicalRegionPath = '/cz/regions/moravskoslezsky'

export function generateStaticParams() {
  return moravskoslezskyCitySlugs.map((slug) => ({ city: slug }))
}

export function generateMetadata({ params }: MoravskoslezskyCityPageProps): Metadata {
  const meta = resolvemoravskoslezskyMetadata(params.city)

  if (!meta) {
    return {
      title: 'Půjčky Moravskoslezský kraj – města',
      description: 'Lokální srovnání půjček v Moravskoslezský kraj_LOCATIVE s licencovanými poskytovateli.',
    }
  }
...
export default function MoravskoslezskyCityPage({ params }: MoravskoslezskyCityPageProps) {
  const city = getmoravskoslezskyCity(params.city)
  ...
  const nearbyCities = city.nearby
    .map((slug) => moravskoslezskyCityMap[slug])
    .filter((entry): entry is MoravskoslezskyCityContent => Boolean(entry))

  const currentYear = new Date().getFullYear()
  const breadcrumbItems = [
    { name: 'Domů', url: '/', position: 1 },
    { name: 'Půjčky Česko', url: '/cz', position: 2 },
    { name: 'Půjčky Moravskoslezský kraj', url: canonicalRegionPath, position: 3 },
