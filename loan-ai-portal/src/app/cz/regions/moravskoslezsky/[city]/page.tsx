...
import { moravskoslezskyCities } from '@/data/cz-moravskoslezsky'
...
export function generateStaticParams() {
  return moravskoslezskyCities.map((city) => ({
    city: city.slug,
  }))
}

export function generateMetadata({ params }: PageProps): Metadata {
  const city = moravskoslezskyCities.find((c) => c.slug === params.city)
  if (!city) return {}
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `/cz/regions/moravskoslezsky/${city.slug}`,
    },
    openGraph: {
      title,
      description,
      url: `/cz/regions/moravskoslezsky/${city.slug}`,
      locale: 'cs_CZ',
      type: 'website',
    },
    twitter: {
...
export default function MoravianSilesianCityPage({ params }: PageProps) {
  const city = moravskoslezskyCities.find((c) => c.slug === params.city)
  if (!city) notFound()
  ...
  const breadcrumbItems = [
    ...
    { name: city.regionName, url: '/cz/regions/moravskoslezsky', position: 3 },
    { name: `Půjčky ${city.name}`, url: `/cz/regions/moravskoslezsky/${city.slug}`, position: 4 },
  ]
  ...
  const financialServiceSchema = {
    ...
    potentialAction: {
      ...
      url: `https://loan-platform.com/cz/regions/moravskoslezsky/${city.slug}#zadost`,
    },
    aggregateRating: {
      ...
      <main className="flex-1 bg-slate-50">
        <header className="border-b border-blue-100 bg-gradient-to-br from-blue-50 via-white to-blue-100">
          <div className="mx-auto max-w-6xl px-4 py-10">
            <Link
              href="/cz/regions/moravskoslezsky"
            ...
            <nav className="mt-2 flex items-center gap-2 text-xs text-blue-900/60" aria-label="Breadcrumb">
              ...
              <span>›</span>
              <Link href="/cz/regions/moravskoslezsky" className="hover:text-blue-600">
                {city.regionName}
              </Link>
              <span>›</span>
          ...
          <div className="grid gap-8 lg:grid-cols-2">
            ...
            <div>
              ...
              <p className="mt-2 text-sm text-blue-900/70">
                Porovnejte nabídky v městech blízko {city.name} v Moravskoslezském kraji
              </p>
              <div className="mt-6 grid gap-4 sm:grid-cols-2">
                {city.nearby.slice(0, 6).map((nearbyCity) => {
                  const nearbyCityData = moravskoslezskyCities.find((c) => c.name === nearbyCity)
                  return nearbyCityData ? (
                    <Card key={nearbyCityData.slug} className="border-blue-100 shadow-sm">
                      ...
                      <CardContent>
                        <Link
                          href={`/cz/regions/moravskoslezsky/${nearbyCityData.slug}`}
              ...
              <div className="mt-6">
                <Link
                  ...
                  className="inline-flex items-center gap-2 text-sm font-semibold text-blue-600 hover:underline"
                >
                  Zobrazit všechny půjčky v Moravskoslezském kraji <span aria-hidden>→</span>
                </Link>
              </div>
            </div>
          </div>
