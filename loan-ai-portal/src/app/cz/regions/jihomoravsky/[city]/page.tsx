...
import type { JihomoravskyCityContent } from '@/data/cz-jihomoravsky'
import { getjihomoravskyCity, jihomoravskyCityMap, jihomoravskyCitySlugs, resolvejihomoravskyMetadata } from '@/data/cz-jihomoravsky'

interface JihomoravskyCityPageProps {
  params: {
    city: string
  }
}

const canonicalRegionPath = '/cz/regions/jihomoravsky'

export function generateStaticParams() {
  return jihomoravskyCitySlugs.map((slug) => ({ city: slug }))
}

export function generateMetadata({ params }: JihomoravskyCityPageProps): Metadata {
  const meta = resolvejihomoravskyMetadata(params.city)

  if (!meta) {
    return {
      title: 'Půjčky Jihomoravský kraj – města',
      description: 'Lokální srovnání půjček v Jihomoravský kraj_LOCATIVE s licencovanými poskytovateli.',
    }
  }
...
export default function JihomoravskyCityPage({ params }: JihomoravskyCityPageProps) {
  const city = getjihomoravskyCity(params.city)
  ...
  const nearbyCities = city.nearby
    .map((slug) => jihomoravskyCityMap[slug])
    .filter((entry): entry is JihomoravskyCityContent => Boolean(entry))

  const currentYear = new Date().getFullYear()
  const breadcrumbItems = [
    { name: 'Domů', url: '/', position: 1 },
    { name: 'Půjčky Česko', url: '/cz', position: 2 },
    { name: 'Půjčky Jihomoravský kraj', url: canonicalRegionPath, position: 3 },
