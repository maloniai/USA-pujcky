...
import type { LibereckyCityContent } from '@/data/cz-liberecky'
import { getlibereckyCity, libereckyCityMap, libereckyCitySlugs, resolvelibereckyMetadata } from '@/data/cz-liberecky'

interface LibereckyCityPageProps {
  params: {
    city: string
  }
}

const canonicalRegionPath = '/cz/regions/liberecky'

export function generateStaticParams() {
  return libereckyCitySlugs.map((slug) => ({ city: slug }))
}

export function generateMetadata({ params }: LibereckyCityPageProps): Metadata {
  const meta = resolvelibereckyMetadata(params.city)

  if (!meta) {
    return {
      title: 'Půjčky Liberecký kraj – města',
      description: 'Lokální srovnání půjček v Liberecký kraj_LOCATIVE s licencovanými poskytovateli.',
    }
  }
...
export default function LibereckyCityPage({ params }: LibereckyCityPageProps) {
  const city = getlibereckyCity(params.city)
  ...
  const nearbyCities = city.nearby
    .map((slug) => libereckyCityMap[slug])
    .filter((entry): entry is LibereckyCityContent => Boolean(entry))

  const currentYear = new Date().getFullYear()
  const breadcrumbItems = [
    { name: 'Domů', url: '/', position: 1 },
    { name: 'Půjčky Česko', url: '/cz', position: 2 },
    { name: 'Půjčky Liberecký kraj', url: canonicalRegionPath, position: 3 },
