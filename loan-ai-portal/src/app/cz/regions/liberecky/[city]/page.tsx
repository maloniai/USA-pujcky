...
import { libereckyCities, libereckyRegionContent } from '@/data/cz-liberecky';
import { notFound } from 'next/navigation';

const currentYear = new Date().getFullYear();

export async function generateStaticParams() {
  return libereckyCities.map((city) => ({
    city: city.slug,
  }));
}

export async function generateMetadata({ params }: { params: { city: string } }): Promise<Metadata> {
  const cityData = libereckyCities.find(c => c.slug === params.city);
  ...
  return {
    title: `Půjčky v ${cityData.name}, Liberecký kraj - Srovnání ${currentYear}`,
    ...
    keywords: `půjčky ${cityData.name}, úvěr ${cityData.name}, nebankovní půjčka, RPSN, Liberecký kraj`,
    openGraph: {
      title: `Půjčky v ${cityData.name}, Liberecký kraj - Srovnání ${currentYear}`,
...
export default function LibereckyCityPage({ params }: { params: { city: string } }) {
  const cityData = libereckyCities.find(c => c.slug === params.city);
  ...
  const financialServiceSchema = {
    ...
    "description": `Srovnání půjček a úvěrů v ${cityData.name}, Liberecký kraj`,
    "areaServed": {
      "@type": "City",
      "name": cityData.name,
      "containedInPlace": {
    ...
    "potentialAction": {
      ...
      "target": {
        "@type": "EntryPoint",
        "urlTemplate": `https://loan-platform.com/cz/regions/liberecky/${cityData.slug}`,
        "actionPlatform": [
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      },
      {
        ...
        "position": 4,
        "name": cityData.name,
        "item": `https://loan-platform.com/cz/regions/liberecky/${cityData.slug}`
      }
    ]
  };
      ...
      <header className="bg-gradient-to-r from-blue-600 to-blue-700 text-white py-12">
        <div className="container mx-auto px-4">
          <nav className="text-sm mb-4 text-blue-100">
            ...
            <span className="mx-2">›</span>
            <Link href="/cz/regions/liberecky" className="hover:text-white">Liberecký kraj</Link>
            <span className="mx-2">›</span>
        ...
        <Card className="shadow-xl bg-gradient-to-r from-blue-50 to-indigo-50">
          <CardContent className="p-8">
            ...
            <div className="flex justify-center mb-6">
              <a
                href={`https://track.fibonad.com/click?campaign_id=${9300 + libereckyCities.findIndex(c => c.slug === cityData.slug)}&pub_id=PUBLISHER_ID`}
                target="_blank"
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid md:grid-cols-3 lg:grid-cols-6 gap-4">
            {cityData.nearbyCities.map((nearbyCity, index) => {
              const nearbyData = libereckyCities.find(c => c.name === nearbyCity);
              return (
                <Link key={index} href={`/cz/regions/liberecky/${nearbyData?.slug || ''}`}>
