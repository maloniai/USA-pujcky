...
import type { UsteckyCityContent } from '@/data/cz-ustecky'
import { getusteckyCity, usteckyCityMap, usteckyCitySlugs, resolveusteckyMetadata } from '@/data/cz-ustecky'

interface UsteckyCityPageProps {
  params: {
    city: string
  }
}

const canonicalRegionPath = '/cz/regions/ustecky'

export function generateStaticParams() {
  return usteckyCitySlugs.map((slug) => ({ city: slug }))
}

export function generateMetadata({ params }: UsteckyCityPageProps): Metadata {
  const meta = resolveusteckyMetadata(params.city)

  if (!meta) {
    return {
      title: 'Půjčky Ústecký kraj – města',
      description: 'Lokální srovnání půjček v Ústecký kraj_LOCATIVE s licencovanými poskytovateli.',
    }
  }
...
export default function UsteckyCityPage({ params }: UsteckyCityPageProps) {
  const city = getusteckyCity(params.city)
  ...
  const nearbyCities = city.nearby
    .map((slug) => usteckyCityMap[slug])
    .filter((entry): entry is UsteckyCityContent => Boolean(entry))

  const currentYear = new Date().getFullYear()
  const breadcrumbItems = [
    { name: 'Domů', url: '/', position: 1 },
    { name: 'Půjčky Česko', url: '/cz', position: 2 },
    { name: 'Půjčky Ústecký kraj', url: canonicalRegionPath, position: 3 },
