...
import type { KarlovarskyCityContent } from '@/data/cz-karlovarsky'
import { getKarlovarskyCity, karlovarskyCityMap, karlovarskyCitySlugs, resolveKarlovarskyMetadata } from '@/data/cz-karlovarsky'

interface KarlovarskyCityPageProps {
  params: {
    city: string
  }
}

const canonicalRegionPath = '/cz/regions/karlovarsky'

export function generateStaticParams() {
  return karlovarskyCitySlugs.map((slug) => ({ city: slug }))
}

export function generateMetadata({ params }: KarlovarskyCityPageProps): Metadata {
  const meta = resolveKarlovarskyMetadata(params.city)

  if (!meta) {
    return {
      title: 'Půjčky Karlovarský kraj – města',
      description: 'Lokální srovnání půjček v Karlovarském kraji s licencovanými poskytovateli.',
    }
  }
...
export default function KarlovarskyCityPage({ params }: KarlovarskyCityPageProps) {
  const city = getKarlovarskyCity(params.city)
  ...
  const nearbyCities = city.nearby
    .map((slug) => karlovarskyCityMap[slug])
    .filter((entry): entry is KarlovarskyCityContent => Boolean(entry))

  const currentYear = new Date().getFullYear()
  const breadcrumbItems = [
    { name: 'Domů', url: '/', position: 1 },
    { name: 'Půjčky Česko', url: '/cz', position: 2 },
    { name: 'Půjčky Karlovarský kraj', url: canonicalRegionPath, position: 3 },
