...
import type { SouthBohemianCityContent } from '@/data/cz-jihocesky'
import { getJihoceskyCity, jihoceskyCityMap, jihoceskyCitySlugs, resolveJihoceskyMetadata } from '@/data/cz-jihocesky'

interface SouthBohemiaCityPageProps {
  params: {
    city: string
  }
}

const canonicalRegionPath = '/cz/regions/jihocesky'

export function generateStaticParams() {
  return jihoceskyCitySlugs.map((slug) => ({ city: slug }))
}

export function generateMetadata({ params }: SouthBohemiaCityPageProps): Metadata {
  const meta = resolveJihoceskyMetadata(params.city)

  if (!meta) {
    return {
      title: 'Půjčky Jihočeský kraj – města',
      description: 'Lokální srovnání půjček v Jihočeském kraji s licencovanými poskytovateli a AI doporučením.',
    }
  }
...
export default function SouthBohemiaCityPage({ params }: SouthBohemiaCityPageProps) {
  const city = getJihoceskyCity(params.city)
  ...
  const nearbyCities = city.nearby
    .map((slug) => jihoceskyCityMap[slug])
    .filter((entry): entry is SouthBohemianCityContent => Boolean(entry))
  ...
  const breadcrumbItems = [
    ...
    { name: 'Půjčky Jihočeský kraj', url: canonicalRegionPath, position: 3 },
      ...
      <main className="flex-1 bg-slate-50">
        ...
        <header className="border-b border-blue-100 bg-gradient-to-br from-blue-50 via-white to-blue-100">
          <div className="mx-auto max-w-5xl px-4 py-16">
            ...
            >
              <span aria-hidden>←</span> Zpět na půjčky Jihočeský kraj
            </Link>
