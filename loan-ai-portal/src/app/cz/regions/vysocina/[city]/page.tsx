...
import type { VysocinaCityContent } from '@/data/cz-vysocina'
import { getvysocinaCity, vysocinaCityMap, vysocinaCitySlugs, resolvevysocinaMetadata } from '@/data/cz-vysocina'

interface VysocinaCityPageProps {
  params: {
    city: string
  }
}

const canonicalRegionPath = '/cz/regions/vysocina'

export function generateStaticParams() {
  return vysocinaCitySlugs.map((slug) => ({ city: slug }))
}

export function generateMetadata({ params }: VysocinaCityPageProps): Metadata {
  const meta = resolvevysocinaMetadata(params.city)

  if (!meta) {
    return {
      title: 'Půjčky Kraj Vysočina – města',
      description: 'Lokální srovnání půjček v Kraj Vysočina_LOCATIVE s licencovanými poskytovateli.',
    }
  }
...
export default function VysocinaCityPage({ params }: VysocinaCityPageProps) {
  const city = getvysocinaCity(params.city)
  ...
  const nearbyCities = city.nearby
    .map((slug) => vysocinaCityMap[slug])
    .filter((entry): entry is VysocinaCityContent => Boolean(entry))

  const currentYear = new Date().getFullYear()
  const breadcrumbItems = [
    { name: 'Domů', url: '/', position: 1 },
    { name: 'Půjčky Česko', url: '/cz', position: 2 },
    { name: 'Půjčky Kraj Vysočina', url: canonicalRegionPath, position: 3 },
