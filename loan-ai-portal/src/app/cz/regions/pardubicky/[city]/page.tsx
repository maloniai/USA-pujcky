...
import { pardubickyCities } from '@/data/cz-pardubicky';
import { VolsorLoanForm } from '@/components/cz/volsor-loan-form';

const currentYear = new Date().getFullYear();

export async function generateStaticParams() {
  return pardubickyCities.map((city) => ({
    city: city.slug,
  }));
}

export async function generateMetadata({ params }: { params: { city: string } }): Promise<Metadata> {
  const city = pardubickyCities.find((c) => c.slug === params.city);
...
export default function PardubickyCityPage({ params }: { params: { city: string } }) {
  const city = pardubickyCities.find((c) => c.slug === params.city);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-slate-100">
      ...
      <section className="bg-gradient-to-r from-blue-600 to-blue-800 text-white py-16 px-4">
        <div className="max-w-7xl mx-auto">
          <nav className="text-sm mb-6 opacity-90">
            ...
            {' › '}
            <Link href="/cz/regions/pardubicky" className="hover:underline">Pardubický kraj</Link>
            {' › '}
            <span>{city.name}</span>
          </nav>
      ...
      <section className="max-w-7xl mx-auto px-4 -mt-8 mb-12 relative z-10">
        <div className="bg-white rounded-2xl shadow-2xl p-6 md:p-8">
          ...
          <VolsorLoanForm affiliate={9320} campaign={`pardubicky-${city.slug}`} partnerName={`Volsor ${city.name}`} />
        </div>
      </section>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 md:p-8">
          ...
          <div className="grid md:grid-cols-3 lg:grid-cols-6 gap-4">
            {city.nearbyCities.map((nearbyCity) => (
              <Link
                key={nearbyCity}
                href={`/cz/regions/pardubicky/${nearbyCity.toLowerCase().replace(/\s+/g, '-').replace(/í/g, 'i').replace(/á/g, 'a').replace(/é/g, 'e').replace(/ý/g, 'y').replace(/ú/g, 'u').replace(/ů/g, 'u').replace(/ě/g, 'e').replace(/š/g, 's').replace(/č/g, 'c').replace(/ř/g, 'r').replace(/ž/g, 'z').replace(/ň/g, 'n').replace(/ť/g, 't').replace(/ď/g, 'd')}`}
          ...
          __html: JSON.stringify({
            ...
            itemListElement: [
              ...
              },
              {
                ...
                position: 3,
                name: city.name,
                item: `https://loan-platform.com/cz/regions/pardubicky/${city.slug}`,
              },
            ],
          })
        }}
