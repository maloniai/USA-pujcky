...
import type { PardubickyCityContent } from '@/data/cz-pardubicky'
import { getpardubickyCity, pardubickyCityMap, pardubickyCitySlugs, resolvepardubickyMetadata } from '@/data/cz-pardubicky'

interface PardubickyCityPageProps {
  params: {
    city: string
  }
}

const canonicalRegionPath = '/cz/regions/pardubicky'

export function generateStaticParams() {
  return pardubickyCitySlugs.map((slug) => ({ city: slug }))
}

export function generateMetadata({ params }: PardubickyCityPageProps): Metadata {
  const meta = resolvepardubickyMetadata(params.city)

  if (!meta) {
    return {
      title: 'Půjčky Pardubický kraj – města',
      description: 'Lokální srovnání půjček v Pardubický kraj_LOCATIVE s licencovanými poskytovateli.',
    }
  }
...
export default function PardubickyCityPage({ params }: PardubickyCityPageProps) {
  const city = getpardubickyCity(params.city)
  ...
  const nearbyCities = city.nearby
    .map((slug) => pardubickyCityMap[slug])
    .filter((entry): entry is PardubickyCityContent => Boolean(entry))

  const currentYear = new Date().getFullYear()
  const breadcrumbItems = [
    { name: 'Domů', url: '/', position: 1 },
    { name: 'Půjčky Česko', url: '/cz', position: 2 },
    { name: 'Půjčky Pardubický kraj', url: canonicalRegionPath, position: 3 },
