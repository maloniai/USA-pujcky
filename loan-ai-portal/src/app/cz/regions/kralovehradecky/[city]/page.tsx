...
import type { KralovehradeckyCityContent } from '@/data/cz-kralovehradecky'
import { getkralovehradeckyCity, kralovehradeckyCityMap, kralovehradeckyCitySlugs, resolvekralovehradeckyMetadata } from '@/data/cz-kralovehradecky'

interface KralovehradeckyCityPageProps {
  params: {
    city: string
  }
}

const canonicalRegionPath = '/cz/regions/kralovehradecky'

export function generateStaticParams() {
  return kralovehradeckyCitySlugs.map((slug) => ({ city: slug }))
}

export function generateMetadata({ params }: KralovehradeckyCityPageProps): Metadata {
  const meta = resolvekralovehradeckyMetadata(params.city)

  if (!meta) {
    return {
      title: 'Půjčky Královéhradecký kraj – města',
      description: 'Lokální srovnání půjček v Královéhradecký kraj_LOCATIVE s licencovanými poskytovateli.',
    }
  }
...
export default function KralovehradeckyCityPage({ params }: KralovehradeckyCityPageProps) {
  const city = getkralovehradeckyCity(params.city)
  ...
  const nearbyCities = city.nearby
    .map((slug) => kralovehradeckyCityMap[slug])
    .filter((entry): entry is KralovehradeckyCityContent => Boolean(entry))

  const currentYear = new Date().getFullYear()
  const breadcrumbItems = [
    { name: 'Domů', url: '/', position: 1 },
    { name: 'Půjčky Česko', url: '/cz', position: 2 },
    { name: 'Půjčky Královéhradecký kraj', url: canonicalRegionPath, position: 3 },
